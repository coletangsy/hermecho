...
def evidence_allows_sentence_first(evidence_dir: str | Path) -> bool:
    """Require a complete, explicitly approved Phase 3 review before promotion."""
    directory = Path(evidence_dir)
    try:
        report = json.loads((directory / "comparison.json").read_text(encoding="utf-8"))
        review = (directory / "review.md").read_text(encoding="utf-8").splitlines()
    except (OSError, UnicodeError, json.JSONDecodeError):
        return False
    if not isinstance(report, dict):
        return False
    artifacts = report.get("artifacts")
    frozen = report.get("frozen_transcription")
    delivery_gates = report.get("delivery_gates")
    media_range = report.get("media_range")
    if (
        report.get("comparison_variable") != "subtitle_delivery"
        or report.get("baseline") != "legacy"
        or report.get("candidate") != "sentence-first"
        or not isinstance(artifacts, dict)
        or not isinstance(frozen, dict)
        or not isinstance(frozen.get("fingerprint"), str)
        or not frozen["fingerprint"]
        or delivery_gates != {"baseline": "passed", "candidate": "passed"}
        or not isinstance(media_range, dict)
        or media_range.get("source_name") != "20251231_w-yGSP1c3bg.mp4"
        or media_range.get("start") != "00:29:30.000"
        or media_range.get("end") != "00:39:30.000"
        or media_range.get("prepared_media") != "media_range.mp4"
        or media_range.get("shared_audio") != "review_composite.mp4"
    ):
        return False
    if not (directory / "media_range.mp4").is_file():
        return False
    for path in artifacts.values():
        if not isinstance(path, str):
            return False
        artifact_path = (directory / path).resolve()
        try:
        ...
        if not artifact_path.is_file():
            return False
    fields = {
        line[2:].split(": ", 1)[0].casefold(): line[2:].split(": ", 1)[1].strip().casefold()
        for line in review
        if line.startswith("- ") and ": " in line
    }
    if any(fields.get(check) in {None, "", "pending"} for check in REVIEW_CHECKS):
        return False
    if fields.get("decision") != "approved" or fields.get("reviewer") in {
        None,
        "",
        "pending",
    } or fields.get("date") in {None, "", "pending"}:
        return False
    try:
        ...
        return False
    try:
        regression_start = review.index("## Timestamped Candidate-only regressions") + 1
        regression_end = review.index("## Human Approval")
    except ValueError:
        return False
    regression_lines = [line.strip() for line in review[regression_start:regression_end] if line.strip()]
    if not regression_lines or any(line.startswith("- PENDING") for line in regression_lines):
        return False
...
def build_delivery_cues(
    ...
    *,
    ...
    align: Optional[Callable[[Dict, DeliveryProfile], Optional[List[Dict]]]] = None,
